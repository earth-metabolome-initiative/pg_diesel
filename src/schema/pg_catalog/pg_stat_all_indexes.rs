//! Submodule for the `pg_catalog.pg_stat_all_indexes` view schema.

diesel::table! {
    /// `pg_catalog.pg_stat_all_indexes` — view showing statistics for all indexes.
    /// Each row represents one index showing statistics about accesses to that specific index.
