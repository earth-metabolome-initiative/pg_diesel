//! Submodule for the `pg_catalog.pg_stat_user_tables` view schema.

diesel::table! {
    /// `pg_catalog.pg_stat_user_tables` — view showing statistics for user-defined tables.
    /// Each row represents one user-defined table showing statistics about accesses to that table.
