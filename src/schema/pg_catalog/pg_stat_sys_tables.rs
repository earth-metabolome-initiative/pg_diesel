//! Submodule for the `pg_catalog.pg_stat_sys_tables` view schema.

diesel::table! {
    /// `pg_catalog.pg_stat_sys_tables` — view showing statistics for system catalog tables.
    /// Each row represents one system catalog table showing statistics about accesses to that table.
