//! Submodule for the `pg_catalog.pg_stat_xact_all_tables` view schema.

diesel::table! {
    /// `pg_catalog.pg_stat_xact_all_tables` — view showing statistics for all tables in the current transaction.
    /// Each row represents one table showing statistics about accesses to that table within the current transaction.
