//! Submodule providing the `PgStatioSysTable` struct representing a row of the
//! `pg_statio_sys_tables` view in `PostgreSQL`.

use diesel::{Queryable, QueryableByName, Selectable};

/// Represents a row from the `pg_statio_sys_tables` view.
#[derive(Queryable, QueryableByName, Selectable, Debug, PartialEq, Eq, Clone, Hash)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[diesel(table_name = crate::schema::pg_catalog::pg_statio_sys_tables::pg_statio_sys_tables)]
pub struct PgStatioSysTable {
    /// Table OID.
