//! Submodule providing the `PgStatXactUserTable` struct representing a row of
//! the `pg_stat_xact_user_tables` view in `PostgreSQL`.

use diesel::{Queryable, QueryableByName, Selectable};

/// Represents a row from the `pg_stat_xact_user_tables` view.
///
/// The `pg_stat_xact_user_tables` view shows statistics about accesses to
/// user-defined tables within the current transaction.
///
...
#[diesel(table_name = crate::schema::pg_catalog::pg_stat_xact_user_tables::pg_stat_xact_user_tables)]
pub struct PgStatXactUserTable {
    /// Table OID.
