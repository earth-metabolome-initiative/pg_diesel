...
#[diesel(table_name = crate::schema::pg_catalog::pg_type::pg_type)]
...
mod tests {
    ...
    fn test_is_composite() {
        let mut ty = dummy_pg_type();
        assert!(!ty.is_composite());

        ty.typcategory = "C".to_string();
        assert!(ty.is_composite());
    }
