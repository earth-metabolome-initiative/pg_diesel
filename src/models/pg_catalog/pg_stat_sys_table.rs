//! Submodule providing the `PgStatSysTable` struct representing a row of the
//! `pg_stat_sys_tables` view in `PostgreSQL`.

use std::time::SystemTime;

use diesel::{Queryable, QueryableByName, Selectable};

/// Represents a row from the `pg_stat_sys_tables` view.
///
/// The `pg_stat_sys_tables` view shows statistics about accesses to system
/// catalog tables.
///
...
#[diesel(table_name = crate::schema::pg_catalog::pg_stat_sys_tables::pg_stat_sys_tables)]
pub struct PgStatSysTable {
    /// Table OID.
